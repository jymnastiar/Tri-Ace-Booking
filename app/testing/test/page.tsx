...
import { X, Copy, Wallet, Check } from 'lucide-react'; // Tambah Check untuk feedback salin
import Link from 'next/link';

export default function KonfirmasiPembayaran({ params }: { params: Promise<{ bookingId: string }> }) {
    const resolvedParams = use(params);
    const bookingId = resolvedParams.bookingId;
    const searchParams = useSearchParams();
    const methodId = searchParams.get('method');

    // 1. STATE UNTUK COUNTDOWN (5 jam 58 menit 53 detik dalam detik)
    const [timeLeft, setTimeLeft] = useState(21533);
    ...
    // Fungsi format waktu (Detik -> HH:MM:SS)
    ...
    const activeMethod = paymentData[methodId as string] || paymentData.BCA;
    ...
    return (
        <div className="h-screen bg-[#F8F9FA] flex flex-col items-center justify-center p-4 md:p-6 overflow-hidden">
            <div className="w-full max-w-5xl bg-white shadow-2xl rounded-3xl overflow-hidden border border-gray-100 flex flex-col max-h-[95vh]">
                ...
                {/* Header Tetap */}
                <div className="bg-[#30A9E3] text-white px-8 py-4 flex justify-between items-center flex-shrink-0">
                    <div className="w-full text-center text-xl md:text-2xl font-bold">Konfirmasi Pembayaran</div>
                    <Link href={`/checkout/${bookingId}`}><X size={24} /></Link>
                </div>

                {/* Sub-header Kirim Uang */}
                <div className="bg-[#EBF8FF] px-8 py-3 flex items-center gap-3 text-[#2B6CB0] font-bold border-b border-blue-100 flex-shrink-0">
                ...
                <div className="p-6 md:p-8 space-y-4 overflow-y-auto">
                    <div className="flex justify-between border-b pb-4 text-lg md:text-xl">
                        ...
                        <span className="text-[#C53030] font-bold">Rp55.000</span>
                    </div>
                    ...
                    {methodId === 'QRIS' ? (
                        <div className="py-4 flex flex-col items-center">
                            ...
                            <div className="p-4 border-2 border-gray-50 rounded-2xl shadow-inner bg-white">
                                <img src="/images/venues/QR.jpeg" alt="QR Code" className="w-40 h-40 md:w-52 md:h-52 object-contain" />
                            </div>
                        </div>
                    ) : (
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b pb-4 gap-2">
                            ...
                            <div className="flex items-center gap-4">
                                <span className="text-[#C53030] font-bold text-xl md:text-2xl tracking-widest">{activeMethod.acc}</span>

                                {/* TOMBOL SALIN OTOMATIS */}
                                <button
                                    onClick={() => handleCopy(activeMethod.acc)}
                                    className={`flex items-center gap-2 text-sm font-bold border-2 px-3 py-1 rounded-lg transition-all ${isCopied ? 'bg-green-500 border-green-500 text-white' : 'text-[#38A169] border-[#38A169] hover:bg-green-50'
                                        }`}
                                >
                    ...
                    <div className="pt-4">
                        <Link
                            href={`/checkout/${bookingId}/confirm/detail`}
